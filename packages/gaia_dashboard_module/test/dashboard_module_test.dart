...
void main() {
  ...
  testWidgets('renders the programme summary surface', (tester) async {
    final controller = GaiaDashboardController(
      client: GaiaIntegrationClient(
        baseUri: Uri.parse('http://127.0.0.1:8765'),
        client: _dashboardMockClient(seenRequests: <String>[]),
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: GaiaProgrammeSummaryView(controller: controller)),
      ),
    );
    await tester.runAsync(controller.refresh);
    await tester.pumpAndSettle();
    ...
    expect(find.text('Provenance manifests'), findsOneWidget);
    expect(find.text('Selected project-alpha'), findsOneWidget);
  });
}
