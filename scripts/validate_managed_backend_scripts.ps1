...
try {
    ...
    foreach ($field in @("flutterVersion", "flutterChannel", "dartVersion", "frameworkRevision", "pythonPackageVersion", "gitBranch", "gitRefState", "gitSha", "backendOwnershipState", "backendCompatibility")) {
        if (-not $status.$field) {
    ...
    if ($cleanupManagedBackend) {
        try {
            Write-Host "Cleaning up managed backend after validation failure."
            & $PSScriptRoot\stop_managed_backend.ps1 -Port $Port -PythonPath $python | Out-Null
        } catch {
            Write-Warning ("Managed backend cleanup after validation failure failed: {0}" -f $_.Exception.Message)
        }
    }
}
